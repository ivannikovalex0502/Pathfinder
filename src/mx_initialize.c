...
static void  print_error_line(int i, t_App *app, char *word1, char *word2);
...
void mx_initialize(t_App *app, char **lines_file) {
    ...
    for (int i = 1; lines_file[i] != NULL; i++) {
        ...
        if (parse_line(lines_file[i],&word1,&word2,&word3)) {
            if(mx_strcmp(word1,word2) == 0)
                print_error_line(i,app,word1,word2);
            mx_guide(word1,word2,app);
            mx_guide_index(app,word1,word2,&word3);
        }
        else
            print_error_line(i,app,word1,word2);
